...
#    s.write("#include <rtt/os/Timer.hpp>\n")
...
# TODO: verify this
    ...
    for rb in sd.trigger_methods.read_buffers:
        s.write("        owner->addEventPort(\"buffer_group_" + str(components_counter) + "_INPORT\", port_buffer_group_" + str(components_counter) + "_in__);\n")
...
#        s.write("            }\n")
    ...
    s.write("    std::vector<std::vector<std::string > > getBufferGroups() const {\n")
    s.write("        std::vector<std::vector<std::string > > vec;\n")
    for rb in sd.trigger_methods.read_buffers:
        s.write("        vec.push_back(std::vector<std::string >());\n")
        all_buffers = rb.obligatory + rb.optional
        for buf_name in all_buffers:
            s.write("        vec.back().push_back(\"" + buf_name + "\");\n")
    s.write("        return vec;\n")
    s.write("    }\n\n")

    s.write("    virtual bool getIntervalInfo(double &min, double &first, double &next) const {\n")
    if sd.trigger_methods.interval:
        s.write("        min = " + str(sd.trigger_methods.interval.min) + ";\n")
        s.write("        bool use_sim_time = false;\n")
        s.write("        ros::param::get(\"/use_sim_time\", use_sim_time);\n")
        s.write("        if (use_sim_time) {\n")
        s.write("            first = " + str(sd.trigger_methods.interval.first_sim) + ";\n")
        s.write("        }\n")
        s.write("        else {\n")
        s.write("            first = " + str(sd.trigger_methods.interval.first) + ";\n")
        s.write("        }\n")
        s.write("        next = " + str(sd.trigger_methods.interval.next) + ";\n")
        s.write("        return true;\n")
    else:
...
#    s.write("        bool timeout = false;\n")
    ...
    components_counter = 0
    for rb in sd.trigger_methods.read_buffers:
        s.write("        bool group_" + str(components_counter) + "_valid = (port_buffer_group_" + str(components_counter) + "_in__.read(buffer_group_" + str(components_counter) + "_in__) == RTT::NewData);\n")
        all_buffers = rb.obligatory + rb.optional
        for buf_name in all_buffers:
            s.write("        " + buf_name + "_data_ = buffer_group_" + str(components_counter) + "_in__." + buf_name + ";\n")
            s.write("        " + buf_name + "_new_data_ = group_" + str(components_counter) + "_valid && buffer_group_" + str(components_counter) + "_in__." + buf_name + "_valid;\n")
        components_counter = components_counter + 1
        for buf_name in rb.obligatory:
            s.write("        result &= " + buf_name + "_new_data_;\n")

#    for p_in in sd.buffers_in:
#        s.write("        if (!" + p_in.alias + "_new_data_ && port_" + p_in.alias + "_in_.read(" + p_in.alias + "_data_, false) == RTT::NewData) {\n")
#        s.write("            " + p_in.alias + "_new_data_ = true;\n")
#        if sd.trigger_methods.onNoData(p_in.alias):
#            s.write("            bool tmp;\n")
#            s.write("            port_" + p_in.alias + "_no_data_trigger_in__.read(tmp);\n")
#        s.write("        }\n")
#        s.write("        else if (!" + p_in.alias + "_new_data_) {\n")
#        if sd.trigger_methods.onNoData(p_in.alias):
#            s.write("            bool tmp;\n")
...
#        s.write("    int " + p.alias + "_no_data_counter_;\n")
    ...
    for rb in sd.trigger_methods.read_buffers:
        ...
        s.write("    RTT::InputPort<" + class_name + " > port_buffer_group_" + str(components_counter) + "_in__;\n")
        s.write("    " + class_name + " buffer_group_" + str(components_counter) + "_in__;\n")
        components_counter = components_counter + 1
...
#    s.write("\n    RTT::InputPort<RTT::os::Timer::TimerId > port_timer_in__;\n")
    s.write("};\n\n")
